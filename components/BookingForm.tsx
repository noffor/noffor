...
const CATEGORY_MAP: Record<string, Record<string, string>> = {
  ...
  Painter: { bn: 'পেইন্টার', ar: 'دهان', hi: 'पेंटर' },
  Carpenter: { bn: 'কার্পেন্টার', ar: 'نجار', hi: 'बढ़ई' },
  Welder: { bn: 'ওয়েল্ডার', ar: 'لحام', hi: 'वेल्डर' },
  Cleaner: { bn: 'ক্লিনার', ar: 'منظف', hi: 'क्लीनर' },
  Cook: { bn: 'রাঁধুনি', ar: 'طباخ', hi: 'रसोइया' },
  Helper: { bn: 'হেল্পার', ar: 'مساعد', hi: 'हेल्पर' },
  Gardener: { bn: 'মালী', ar: 'بستاني', hi: 'माली' },
  Housemaid: { bn: 'গৃহকর্মী', ar: 'خادمة', hi: 'हाउसमेड' },
  Nanny: { bn: 'আয়া', ar: 'مربية', hi: 'नैनी' },
  'Office Assistant': { bn: 'অফিস সহকারী', ar: 'مساعد مكتبي', hi: 'ऑफिस असिस्टेंट' },
  Receptionist: { bn: 'রিসেপশনিস্ট', ar: 'موظف استقبال', hi: 'रिसेप्शनिस्ट' },
  Salesman: { bn: 'সেলসম্যান', ar: 'بائع', hi: 'सेल्समैन' },
  Cashier: { bn: 'ক্যাশিয়ার', ar: 'كاشير', hi: 'कैशियर' },
  'Security Guard': { bn: 'সিকিউরিটি গার্ড', ar: 'حارس أمن', hi: 'सिक्योरिटी गार्ड' },
  Nurse: { bn: 'নার্স', ar: 'ممرض', hi: 'नर्स' },
  Pharmacist: { bn: 'ফার্মাসিস্ট', ar: 'صيدلي', hi: 'फार्मासिस्ट' },
  'Lab Technician': { bn: 'ল্যাব টেকনিশিয়ান', ar: 'فني مختبر', hi: 'लैब तकनीशियन' },
  Physiotherapist: { bn: 'ফিজিওথেরাপিস্ট', ar: 'معالج طبيعي', hi: 'फिजियोथेरेपिस्ट' },
  Mechanic: { bn: 'মেকানিক', ar: 'ميكانيكي', hi: 'मैकेनिक' },
  Tailor: { bn: 'দর্জি', ar: 'خياط', hi: 'दर्जी' },
  Barista: { bn: 'বারিস্তা', ar: 'باريستا', hi: 'बरिस्ता' },
  Photographer: { bn: 'ফটোগ্রাফার', ar: 'مصور', hi: 'फोटोग्राफर' },
  'CCTV Technician': { bn: 'সিসিটিভি টেকনিশিয়ান', ar: 'فني كاميرات', hi: 'CCTV तकनीशियन' },
  'Gypsum Carpenter': { bn: 'জিপসাম কার্পেন্টার', ar: 'نجار جبس', hi: 'जिप्सम कारपेंटर' },
  'Tiles Mason': { bn: 'টাইলস মিস্ত্রি', ar: 'عامل تبليط', hi: 'टाइल्स मिस्त्री' },
  Blacksmith: { bn: 'কামার', ar: 'حداد', hi: 'लोहार' },
  'General Labour': { bn: 'সাধারণ শ্রমিক', ar: 'عامل عام', hi: 'सामान्य श्रमिक' },
  'Steel Fixer': { bn: 'স্টিল ফিক্সার', ar: 'مثبت حديد', hi: 'स्टील फिक्सर' },
  Scaffolder: { bn: 'স্ক্যাফোল্ডার', ar: 'عامل سقالات', hi: 'स्कैफोल्डर' },
  'Heavy Driver': { bn: 'ভারী ড্রাইভার', ar: 'سائق ثقيل', hi: 'भारी ड्राइवर' },
  'Forklift Operator': { bn: 'ফর্কলিফট অপারেটর', ar: 'مشغل رافعة', hi: 'फोर्कलिफ्ट ऑपरेटर' },
  'Crane Operator': { bn: 'ক্রেন অপারেটর', ar: 'مشغل رافعة', hi: 'क्रेन ऑपरेटर' },
  'Pipe Fitter': { bn: 'পাইপ ফিটার', ar: 'مركب أنابيب', hi: 'पाइप फिटर' },
  Waiter: { bn: 'ওয়েটার', ar: 'نادل', hi: 'वेटर' },
  'Hotel Housekeeping': { bn: 'হোটেল হাউসকিপিং', ar: 'تدبير فندقي', hi: 'होटल हाउसकीपिंग' },
  Beautician: { bn: 'বিউটিশিয়ান', ar: 'خبيرة تجميل', hi: 'ब्यूटीशियन' },
  Barber: { bn: 'নাপিত', ar: 'حلاق', hi: 'नाई' },
};
  ...
  <div className="sticky top-0 bg-white border-b p-3 sm:p-4 flex items-center justify-between z-10 rounded-t-2xl">
    <div className="flex items-center gap-3 min-w-0">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[11px] sm:text-xs text-gray-500 flex items-center gap-1"><Briefcase size={10} /> {translateCategory(worker.category, lang)}{worker.rating && <><span className="mx-1">•</span><Star size={10} className="text-yellow-500" /> {worker.rating}</>}{!worker.rating && <><span className="mx-1">•</span>{txt.new}</>}</p>
      </div>
    </div>
...
const InputField = React.memo(({ label, value, onChange, type = 'text', icon, placeholder }: { label: string; value: string; onChange: (v: string) => void; type?: string; icon?: React.ReactNode; placeholder?: string; }) => (
  <div>
    ...
    <div className="relative">
      <input value={value} onChange={e => onChange(e.target.value)} type={type} placeholder={placeholder} className={`w-full px-3 py-2.5 ${icon ? 'pl-9' : ''} border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition-all`} />
...
export default function BookingForm({ worker, isOpen, onClose, country, lang }: BookingFormProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cached) { const p = JSON.parse(cached); if (Date.now() - p.t < 300000) { startTransition(() => setUserLocation({ lat: p.lat, lng: p.lng })); return; } }
    } catch {}
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((pos) => { if (!alive.current) return; const loc = { lat: pos.coords.latitude, lng: pos.coords.longitude }; startTransition(() => setUserLocation(loc)); try { sessionStorage.setItem('user_loc', JSON.stringify({ ...loc, t: Date.now() })); } catch {} }, () => {}, { timeout: CONFIG.GEOLOCATION_TIMEOUT, maximumAge: 300000 });
    }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const bookingData = {
      ...
      special_instructions: '', contact_phone: phone.trim(), status: 'pending', employer_id: null,
    };
    for (let r = 0; r <= CONFIG.MAX_RETRY; r++) {
      try {
        const { error } = await supabase.from('bookings').insert(bookingData).select().single();
        if (error) throw error;
        supabase.from('notifications').insert({ user_id: worker.id, title: 'New Booking Request', message: `${name} - ${cleanAmount} ${currencyText}`, type: 'booking_request', is_read: false }).then(() => {});
        onClose();
        return;
      } catch (err: any) {
        if (r >= CONFIG.MAX_RETRY) { console.error('Booking error:', err); alert(txt.error); }
        else { await new Promise(res => setTimeout(res, 500 * (r + 1))); }
      }
    }
    setSubmitting(false);
  }, [name, phone, location, description, offeredAmount, paymentMethod, startDate, startTime, duration, userLocation, worker, distanceKm, etaMinutes, submitting, txt, currencyText, onClose]);
  ...
  const reviewItems = useMemo(() => {
    ...
    if (startDate) {
      ...
      const day = dateObj.getDate(); const year = dateObj.getFullYear();
          ...
          {step === 2 && (
            <div className="space-y-3 pt-2">
              ...
              <div>
                ...
                {worker.expected_salary && <p className="text-[11px] sm:text-xs text-gray-400 mt-1">{txt.expects} {translateNumber(String(worker.expected_salary).replace(/[^0-9]/g, ''), lang)} {currencyText}</p>}
              </div>
              <div>

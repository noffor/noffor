...
// 🚀 SUPER SONIC • 1B USERS • UBER STYLE • AUTO FIND NEXT WORKER
// ✅ Worker Reject = Auto Map Open (Like Uber)
"use client";
...
const T: Record<string, Record<string, string>> = {
  en: {
    ...
    workerAccepted: '🎉 Worker Accepted!', workerRejected: '❌ Rejected - Finding others...',
    ...
    offerSent: 'Booking sent! Waiting...',
    autoSearching: 'Auto-searching nearby workers...',
  },
  bn: {
    ...
    workerAccepted: '🎉 শ্রমিক রাজি!', workerRejected: '❌ রাজি নয় - অন্য খুঁজছে...',
    ...
    offerSent: 'বুকিং পাঠানো! অপেক্ষা...',
    autoSearching: 'স্বয়ংক্রিয়ভাবে শ্রমিক খুঁজছে...',
  },
  ar: {
    ...
    workerAccepted: '🎉 قبل العامل!', workerRejected: '❌ رفض - جاري البحث...',
    ...
    offerSent: 'تم الإرسال! انتظار...',
    autoSearching: 'جاري البحث تلقائياً...',
  },
  hi: {
...
export default function HomeTabs({ country, lang }: Props) {
  ...
  const [online, setOnline] = useState(() => {
    if (typeof window !== 'undefined') { try { const s = localStorage.getItem(STORAGE_KEY); if (s) return JSON.parse(s)?.isOnline || false; } catch {} }
    return false;
  });
  ...
  const handleWorkerSelect = useCallback((worker: any) => {
    ...
    setShowBookingForm(true); setShowMap(false);
  }, []);

  // ✅ FIX: Auto Map Open on Reject
  const handleBookingFormClose = useCallback(() => {
    setShowBookingForm(false); setBookingState('waiting');
        ...
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'bookings', filter: `worker_id=eq.${selectedWorker.worker_id}` },
          (payload: any) => {
            ...
            if (status === 'accepted') { setBookingState('accepted'); showToast(tr.workerAccepted, 'success'); }
              ...
              // ✅ AUTO: 2.5 sec পর Map Open
              rejectTimerRef.current = setTimeout(() => {
                setBookingState('idle'); setSelectedWorker(null); setShowMap(true);
              }, 2500);
            }
          }).subscribe();
      ...
      {bookingState === 'accepted' && selectedWorker && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={handleCloseBooking}>
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 space-y-4 text-center animate-bounce-in" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-gray-50 rounded-xl p-4 text-left space-y-2"><div className="flex items-center gap-3">{selectedWorker.profile?.photo_url ? <img src={selectedWorker.profile.photo_url} className="w-12 h-12 rounded-full" /> : <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center"><Users size={20} /></div>}<div><p className="font-bold">{selectedWorker.profile?.name}</p><p className="text-sm text-gray-500">{selectedWorker.profile?.category}</p></div></div><div className="flex items-center gap-2 text-sm text-gray-600 pt-2 border-t"><Clock size={14} /><span>{tr.arrivingIn} ~{selectedWorker.eta || 15} {tr.min}</span></div></div>
            <div className="space-y-2"><a href={`tel:${selectedWorker.profile?.phone || ''}`} className="w-full py-3 bg-green-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[0.98]"><Phone size={16} />{tr.callWorker}</a><button onClick={() => { setShowChat(true); handleCloseBooking(); }} className="w-full py-3 bg-blue-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[0.98]"><MessageCircle size={16} />{tr.chatWithWorker}</button><button onClick={() => setBookingState('tracking')} className="w-full py-3 bg-purple-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 active:scale-[0.98]"><MapPin size={16} />{tr.trackLive}</button></div>
            <button onClick={handleCloseBooking} className="text-red-500 text-sm">{tr.cancelBooking}</button>
          </div>
        </div>
      )}

      {/* ✅ AUTO REJECT MODAL - 2.5 sec then Map */}

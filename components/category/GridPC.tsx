// components/category/GridPC.tsx - ১ বিলিয়ন ইউজার • সুপারসনিক • ৪ ভাষা
"use client";
...
// ═══════════════════════════════════════════════════════════
// GridPC (Memoized • 1B Ready)
// ═══════════════════════════════════════════════════════════
const GridPC=React.memo(({profiles,country,lang,totalCount,categoryName}:Props)=>{
  ...
  return(
    <div style={{contain:'layout style paint'}}>
      ...
      {hasMore&&!error&&(
        <div className="text-center mt-6">
          <button onClick={loadMore} disabled={loading}
            className="px-6 py-2.5 bg-orange-600 text-white rounded-xl text-sm font-semibold hover:bg-orange-700 disabled:opacity-50 transition-all active:scale-[0.98] flex items-center gap-2 mx-auto shadow-sm">
      ...
      {!hasMore&&items.length>0&&(
        <p className="text-center text-xs text-gray-400 mt-6">{tr.noMore}</p>
      )}
    </div>
  );
});

GridPC.displayName='GridPC';

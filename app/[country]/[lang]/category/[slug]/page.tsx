...
export default function CategoryPage(){
      ...
      if(e.isIntersecting&&hasMore&&!loadingMore&&!loading){
        ...
        tid=setTimeout(()=>{
          ...
          loadProfiles(np,true).then(d=>{
            if(aliveRef.current)startTransition(()=>{
              setProfiles(p=>[...p,...d]);
              setPage(np);
  ...
  const skeletons=useMemo(()=>
    Array.from({length:CONFIG.ITEMS_PER_PAGE}).map((_,i)=>(
        ...
        {error&&(
          <div className="text-center py-12">
            ...
            <button 
              ...
              className="px-4 py-2 bg-orange-600 text-white rounded-lg text-sm font-semibold hover:bg-orange-700 active:scale-[0.98] transition-all flex items-center gap-2 mx-auto"
            >

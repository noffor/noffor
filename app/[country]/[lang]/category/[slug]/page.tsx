...
// 🚀 SUPER SONIC • 42 CATEGORIES • PNG IMAGES • MOBILE + PC • SIDEBAR ONLY • FIXED
"use client";
import React, { useState, useEffect, useCallback, useRef, startTransition, lazy, Suspense } from 'react';
import { useParams } from 'next/navigation';
import { supabase } from '@/lib/supabase';
import { Crosshair, Wifi, WifiOff, X, Search, ArrowLeft } from 'lucide-react';
...
// ═══════════════════════════════════════════════════════════
// AllCategoriesPage — PC-তে 6 columns, Mobile-এ 3 columns
// ═══════════════════════════════════════════════════════════
      ...
      <div className="px-4 py-3 border-b bg-white sticky top-[57px] z-10">
        <div className="relative max-w-md mx-auto">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-3 bg-gray-50 border rounded-xl text-sm focus:border-orange-500 outline-none"
            autoFocus
          />
        </div>
      </div>

      {/* Grid — ✅ PC: 6 columns, Mobile: 3 columns */}
...
const LoginToast = React.memo(({ message, dismiss, onClose }: { message: string; dismiss: string; onClose: () => void }) => (
  <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-orange-500 text-white px-4 py-3 rounded-xl shadow-lg flex items-center gap-3 animate-slideDown max-w-sm w-[90%]">
    <WifiOff size={18} /><span className="text-sm font-medium flex-1">{message}</span>
...
// ═══════════════════════════════════════════════════════════
// ✅ Fallback Banner — একই সাইজ (h-48 lg:h-64)
// ═══════════════════════════════════════════════════════════
const FallbackBanner = React.memo(({ lang }: { lang: string }) => (
  <div className="relative rounded-xl overflow-hidden bg-gradient-to-r from-gray-800 to-gray-900 h-48 lg:h-64 group mb-3 shadow-lg">
    <div className="absolute inset-0 flex items-center justify-center">
      <div className="text-center">
        <h2 className="text-2xl lg:text-3xl font-bold text-white mb-2">
          {lang === 'bn' ? 'নির্ভরযোগ্য শ্রমিক খুঁজুন' : lang === 'ar' ? 'اعثر على عمال موثوقين' : lang === 'hi' ? 'विश्वसनीय श्रमिक खोजें' : 'Find Reliable Workers'}
        </h2>
        <p className="text-gray-300 text-sm lg:text-base">
          {lang === 'bn' ? 'দ্রুত, নিরাপদ ও সহজ' : lang === 'ar' ? 'سريع وآمن وسهل' : lang === 'hi' ? 'तेज़, सुरक्षित और आसान' : 'Fast, Safe & Easy'}
        </p>
      </div>
    </div>
  </div>
));
FallbackBanner.displayName = 'FallbackBanner';

// ═══════════════════════════════════════════════════════════
// PC Layout — ✅ Sidebar only, NO extra Category Grid
// ═══════════════════════════════════════════════════════════
const PCLayout = React.memo(({ country, lang, showMap, userLocation, online, toggleMap, toggleOnline, onMoreClick }: {
  ...
  <div className="hidden lg:block">
    ...
    {showMap && userLocation ? (
      <Suspense fallback={<div className="h-64 bg-gray-200 animate-pulse rounded-xl mb-3" />}>
        <LiveWorkerMap country={country} lang={lang} userLat={userLocation.lat} userLng={userLocation.lng} />
      </Suspense>
    ) : (
      <Suspense fallback={<FallbackBanner lang={lang} />}>
...
const MobileLayout = React.memo(({ country, lang, onMoreClick }: { country: string; lang: string; onMoreClick: () => void }) => (
  <div className="lg:hidden">
    ...
    {/* ✅ মোবাইলে PNG Image Category Grid */}
    <div className="bg-white rounded-xl p-3 border mt-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <button
          onClick={onMoreClick}
          className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-2 text-center border border-orange-200 hover:shadow-md transition-all active:scale-95"
        >
...
function HomePage() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { if (!alive.current) return; const loc = { lat: pos.coords.latitude, lng: pos.coords.longitude }; startTransition(() => setUserLocation(loc)); setStorageItem(CONFIG.LOCATION_CACHE_KEY, JSON.stringify({ ...loc, t: Date.now() })); },
      () => { if (alive.current) { const defaultLoc = CONFIG.DEFAULT_LOCATIONS[country] || CONFIG.DEFAULT_LOCATIONS.qa; startTransition(() => setUserLocation(defaultLoc)); } },
      { timeout: CONFIG.GEOLOCATION_TIMEOUT, maximumAge: CONFIG.GPS_MAX_AGE, enableHighAccuracy: false }
    );
  }, [country]);

  useEffect(() => { return () => { if (loginToastTimerRef.current) clearTimeout(loginToastTimerRef.current); }; }, []);
  ...
  const toggleOnline = useCallback(async () => {
    ...
    if (!workerData) {
      if (!toastShownRef.current) { toastShownRef.current = true; setShowLoginToast(true); if (loginToastTimerRef.current) clearTimeout(loginToastTimerRef.current); loginToastTimerRef.current = setTimeout(() => { setShowLoginToast(false); toastShownRef.current = false; }, 3000); }
      return;
    }
    const nextState = !online; startTransition(() => setOnline(nextState)); setStorageItem(CONFIG.ONLINE_CACHE_KEY, JSON.stringify(nextState));
    try { const profile = JSON.parse(workerData); const updateData: any = { is_online: nextState }; if (nextState) { updateData.is_public = true; } await supabase.from('profiles').update(updateData).eq('id', profile.id); }
    catch (err) { console.error('Toggle online error:', err); startTransition(() => setOnline(!nextState)); }
  }, [online]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {showLoginToast && (<LoginToast message={T[lang]?.loginRequired || T.en.loginRequired} dismiss={T[lang]?.dismiss || T.en.dismiss} onClose={() => { setShowLoginToast(false); toastShownRef.current = false; }} />)}
      <main className="max-w-7xl mx-auto px-3 lg:px-4 py-3">
        {isDesktop && (<PCLayout country={country} lang={lang} showMap={showMap} userLocation={userLocation} online={online} toggleMap={toggleMap} toggleOnline={toggleOnline} onMoreClick={handleMoreClick} />)}
...
export default function HomePageWithErrorBoundary() {
  return (<ErrorBoundary lang="en"><HomePage /></ErrorBoundary>);
}
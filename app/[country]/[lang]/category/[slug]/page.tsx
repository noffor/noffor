...
// 🚀 SUPER SONIC • PER-CATEGORY SLIDER • BIG BANNER • NO CRASH • 1B READY
"use client";
...
import {Loader2,AlertCircle,RefreshCw,Package,Image as ImageIcon,Filter,ChevronDown,X,SlidersHorizontal,ArrowUpDown,ChevronLeft,ChevronRight,Star,MapPin,Briefcase} from 'lucide-react';
...
const T:Record<string,Record<string,string>>={
  en:{
    ...
    featuredWorker:'Featured Worker',
  },
  bn:{
...
// কনফিগ
// ═══════════════════════════════════════════════════════════
const CONFIG={ITEMS_PER_PAGE:12,CACHE_TTL:30000,RETRY_MAX:2,SLIDER_COUNT:10};
...
// ═══════════════════════════════════════════════════════════
// ✅ WORKER SLIDER — BIG SIZE • PER CATEGORY • 10 WORKERS
// ═══════════════════════════════════════════════════════════
const WorkerSlider = React.memo(({ workers, lang, categoryName }: { 
  workers: any[]; 
  lang: string; 
  categoryName: string;
}) => {
  ...
  const goNext = useCallback(() => setCurrentIndex(prev => (prev + 1) % workers.length), [workers.length]);
  const goPrev = useCallback(() => setCurrentIndex(prev => (prev - 1 + workers.length) % workers.length), [workers.length]);
  ...
  const workerPhoto = currentWorker.photo_url && !imgErrors.has(currentIndex) 
    ? currentWorker.photo_url 
    : '/default-avatar.png';

  return (
    <div 
      className="relative rounded-2xl overflow-hidden h-52 sm:h-64 lg:h-72 xl:h-80 group mb-4 shadow-xl shadow-black/10"
    ...
    >
      ...
      <img
        src={workerPhoto}
        alt={currentWorker.name || categoryName}
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-5 lg:p-7">
        {/* Featured Badge */}
        <div className="mb-2">
          <span className="bg-orange-500/90 backdrop-blur-sm text-white text-[10px] lg:text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
            {tr.featuredWorker}
          </span>
        </div>

        {/* Worker Name */}
        <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white drop-shadow-lg mb-1">
          {currentWorker.name || categoryName}
        </h3>

        {/* Worker Info */}
        <div className="flex items-center gap-3 text-white/80 text-xs lg:text-sm">
          <span className="flex items-center gap-1">
            <Briefcase size={12} className="text-orange-400" />
            {translateCategory(currentWorker.category || categoryName, lang)}
          </span>
          {currentWorker.city && (
            <>
      ...
      {/* Worker Count + Category Badge */}
      <div className="absolute top-4 right-4 flex items-center gap-2">
        <span className="bg-white/20 backdrop-blur-sm text-white text-[10px] font-medium px-2 py-1 rounded-full hidden sm:block">
          {translateCategory(categoryName, lang)}
        </span>
        <span className="bg-black/30 backdrop-blur-md text-white text-xs px-3 py-1.5 rounded-full border border-white/20">
          {workers.length} {lang === 'bn' ? 'জন' : lang === 'ar' ? 'عامل' : lang === 'hi' ? 'श्रमिक' : 'workers'}
        </span>
      </div>
      ...
      {workers.length > 1 && (
        <>
          <button 
            onClick={goPrev} 
            className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/30 backdrop-blur-sm text-white p-2.5 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-black/50 hover:scale-110"
            aria-label="Previous"
          >
            <ChevronLeft size={22} />
          </button>
          <button 
            onClick={goNext} 
            className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/30 backdrop-blur-sm text-white p-2.5 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-black/50 hover:scale-110"
            aria-label="Next"
          >
...
WorkerSlider.displayName = 'WorkerSlider';

// ═══════════════════════════════════════════════════════════
// CATEGORY-WISE FILTER CONFIG
// ═══════════════════════════════════════════════════════════
interface FilterConfig {
  visa: boolean; accommodation: boolean; gender: boolean;
  language: boolean; license: boolean; tools: boolean;
...
const DEFAULT_CONFIG: FilterConfig = { visa: false, accommodation: false, gender: false, language: false, license: false, tools: false };
...
export default function CategoryPage() {
  ...
  const [sliderWorkers, setSliderWorkers] = useState<any[]>([]);
  ...
  // ✅ Load Slider Workers — Top 10 verified with photo from THIS category
  const loadSliderWorkers = useCallback(async () => {
    try {
      const { data } = await supabase
        .from('profiles')
        .select('id, name, photo_url, category, city, rating')
        ...
        .limit(CONFIG.SLIDER_COUNT);

      if (data && aliveRef.current) {
        setSliderWorkers(data);
      }
    } catch (err) {
  ...
  const loadProfiles = useCallback(async (pageNum: number, append: boolean) => {
    ...
    if (!append) {
      ...
      if (cached && Date.now() - cached.timestamp < CONFIG.CACHE_TTL) {
        startTransition(() => {
          ...
          setHasMore(pageNum * CONFIG.ITEMS_PER_PAGE + CONFIG.ITEMS_PER_PAGE < cached.total);
        });
        return cached.data;
      }
    }
    ...
    try {
      const from = pageNum * CONFIG.ITEMS_PER_PAGE;
      ...
      startTransition(() => {
        setTotalCount(total);
        setHasMore(from + CONFIG.ITEMS_PER_PAGE < total);
      });
      retryRef.current = 0;
      return result;
    } catch {
      if (retryRef.current < CONFIG.RETRY_MAX) { retryRef.current++; return loadProfiles(pageNum, append); }
      if (aliveRef.current) startTransition(() => setError(true));
      return [];
    }
  ...
  const resetAndLoad = useCallback(() => {
    ...
    loadProfiles(0, false).then(d => {
      if (aliveRef.current) startTransition(() => { setProfiles(d); setPage(0); setLoading(false); });
    });
    loadSliderWorkers();
  }, [loadProfiles, loadSliderWorkers]);

  useEffect(() => { aliveRef.current = true; resetAndLoad(); return () => { aliveRef.current = false; }; }, [resetAndLoad]);

  // Realtime
  useEffect(() => {
    const channel = supabase.channel(`cat:${categoryName}:${Date.now()}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'profiles', filter: `country=eq.${country}` }, (payload: any) => {
        if (payload.new.role === 'labor' && payload.new.category === categoryName && payload.new.is_public === true && payload.new.is_verified === true && aliveRef.current) {
          startTransition(() => {
            ...
            setTotalCount(p => p + 1);
            setSliderWorkers(p => { if (p.length < CONFIG.SLIDER_COUNT) return [...p, payload.new]; return p; });
          });
        }
      }).subscribe();
    ...
    observerRef.current = new IntersectionObserver(([e]) => {
      if (e.isIntersecting && hasMore && !loadingMore && !loading) {
        ...
        tid = setTimeout(() => {
          ...
          const np = page + 1;
          loadProfiles(np, true).then(d => {
            if (aliveRef.current) startTransition(() => { setProfiles(p => [...p, ...d]); setPage(np); setLoadingMore(false); });
          });
        }, 100);
      }
  ...
  const skeletons = useMemo(() => Array.from({ length: CONFIG.ITEMS_PER_PAGE }).map((_, i) => (
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 lg:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-3 lg:px-4 py-3">
        ...
        {/* ✅ WORKER SLIDER — BIG SIZE • PER CATEGORY */}
        {sliderWorkers.length > 0 ? (
          <WorkerSlider workers={sliderWorkers} lang={lang} categoryName={categoryName} />
        ) : (
          /* Fallback Banner — Same Big Size */
          <div className="relative rounded-2xl overflow-hidden mb-4 shadow-xl shadow-black/10 h-52 sm:h-64 lg:h-72 xl:h-80">
            {!imgError ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-gray-800 via-gray-700 to-orange-900 flex flex-col items-center justify-center gap-4">
                <div className="w-16 h-16 lg:w-20 lg:h-20 bg-white/10 rounded-2xl flex items-center justify-center">
                  <ImageIcon size={32} className="text-white/30" />
                </div>
                <span className="text-white/50 text-base lg:text-lg font-bold">{translateCategory(categoryName, lang)}</span>
              </div>
            )}
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />
            <div className="absolute inset-0 p-5 lg:p-7 flex flex-col justify-end">
              <p className="text-white/70 text-[10px] lg:text-xs font-medium mb-1 uppercase tracking-wider">
                {isMainCategory ? tr.mainCategories : tr.otherCategories}
              </p>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-white drop-shadow-lg">
                {translateCategory(categoryName, lang)}
              </h1>
              <p className="text-white/50 text-xs lg:text-sm mt-1">{country?.toUpperCase()}</p>
              {!loading && (
                <div className="mt-3">
        ...
        {/* ✅ FILTERS + SORT — PC */}
        <div className="hidden lg:block bg-white rounded-xl border shadow-sm mb-3 overflow-hidden sticky top-16 z-20">
          <div className="p-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button onClick={() => setShowMoreFilters(!showMoreFilters)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all active:scale-95 flex items-center gap-1 ${showMoreFilters ? 'bg-orange-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
        ...
        {/* ✅ FILTERS + SORT — Mobile */}
        <div className="lg:hidden bg-white rounded-xl border shadow-sm mb-3 overflow-hidden sticky top-14 z-20">
          <div className="p-2">
            <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide pb-1">
              ...
              <button onClick={() => setShowMoreFilters(!showMoreFilters)} className={`px-2.5 py-1 rounded-full text-[10px] font-medium whitespace-nowrap transition-all active:scale-95 flex items-center gap-1 ${showMoreFilters ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
        ...
        {/* Loading */}
        {loading && <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 lg:gap-3">{skeletons}</div>}
        ...
        {/* Grid — PC: 6 cols, Tablet: 3 cols, Mobile: 2 cols */}
        {!loading && !error && profiles.length > 0 && (
          <>
            <div className="hidden lg:grid grid-cols-6 gap-3">
              {profiles.map(p => <ProfileCard key={p.id} profile={p} href={`${rest}/profile/${p.id}`} lang={lang} />)}
            </div>
            <div className="hidden sm:grid sm:grid-cols-3 lg:hidden gap-2">
              {profiles.map(p => <ProfileCard key={p.id} profile={p} href={`${rest}/profile/${p.id}`} lang={lang} />)}
            </div>
